...
genomic_assays_matched_time_points_list_merge <- genomic_assays_matched_time_points_list %>% 
  inner_join(pbta, by=c('Kids_First_Participant_ID')) %>%
...
genomic_assays_matched_time_points_list_merge %>%
  group_by(Kids_First_Participant_ID, match_id_multiple_samples, tumor_descriptor) %>%
  tally() %>%
  regulartable() %>%
...
maf_files_Deceased <- snv %>% 
  left_join(genomic_assays_matched_time_points_list_merge, by=c('Kids_First_Biospecimen_ID')) %>%
