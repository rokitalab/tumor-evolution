...
stat.test <- df_plot_cgGFAC %>%
  group_by(cgGFAC) %>%
  pairwise_t_test(log10_tmb ~ timepoint_cgGFAC_n, 
                  paired = FALSE, 
                  p.adjust.method = "bonferroni") %>% 
  write_tsv(file.path(results_dir, "stat_test.tsv"))


test <- wilcox.test(df_plot_cgGFAC$log10_tmb ~ df_plot_cgGFAC$timepoint_cgGFAC_n,
  #alternative = "less",
  paired = FALSE
)
test
...
stat.test2 <- 
  pairwise.wilcox.test(df_plot_cgGFAC$log10_tmb ~ df_plot_cgGFAC$timepoint_cgGFAC_n, 
                  paired = FALSE, 
                  p.adjust.method = "bonferroni") %>% 
  write_tsv(file.path(results_dir, "stat_test.tsv"))
...
stat.test <- stat.test %>% add_xy_position(x = "timepoint_cgGFAC_n")
