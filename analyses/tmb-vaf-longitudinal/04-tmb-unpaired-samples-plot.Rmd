...
```{r summary-statistics, fig.width = 8, fig.height = 5, fig.fullwidth = TRUE}
...
for (i in seq_along(cancer_groups)) {
  ...
  # Add add_xy_position
  stat.test <- stat.test %>% 
    add_xy_position(x = "timepoint_cgGFAC_n", step.increase = .08) 
  ...
  # Save the plot
  ggsave(filename = paste0(cancer_groups[i], "-TMB-Bxp-stat-test1.pdf"), 
         path = plots_dir, 
         width = 8, 
         height = 5, 
         device = "pdf", 
