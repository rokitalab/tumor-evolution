...
df_plot <- readr::read_tsv(pbta_file, guess_max = 100000, show_col_types = FALSE) %>%
  ...
  dplyr:::mutate(short_histology_sum = ifelse(short_histology == "HGAT", "HGG",
                                              ifelse(short_histology == "LGAT", "LGG", 
                                                     ifelse(short_histology == "Ependymoma", "Ependymoma", "Other"))),
                 log10_tmb = log10(tmb),
...
df_plot <- df_plot %>%
  dplyr::count(short_histology_sum) %>%
  dplyr::distinct() %>%
  dplyr::select(short_histology_sum, n) %>%
  # Create wrapped with (n=X) factor column for cancer groups
  dplyr::mutate(cancer_group_display_n = glue::glue("{short_histology_sum} (N={n})")) %>%
