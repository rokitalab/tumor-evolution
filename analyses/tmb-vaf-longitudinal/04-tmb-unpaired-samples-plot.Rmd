...
```{r summary-statistics, fig.width = 6, fig.height = 5, fig.fullwidth = TRUE}
...
cancer_groups <- unique(as.character(df_plot_cgGFAC$cgGFAC))
cancer_groups <- sort(samples, decreasing = FALSE)
print(cancer_groups)
...
for (i in seq_along(cancer_groups)) {
  ...
  # Add add_xy_position
  stat.test <- stat.test %>% 
    add_xy_position(x = "timepoint_cgGFAC_n", dodge = 1, step.increase = .02) 

  # Create bxp
  print(ggboxplot(df_sub,
                  x = "tumor_descriptor",
                  y = "log10_tmb",
  ...
  # Save the plot
  ggsave(filename = paste0(cancer_groups[i], "-TMB-Bxp-stat-test.pdf"), 
         path = plots_dir, 
         width = 6, 
         height = 5, 
         device = "pdf", 
