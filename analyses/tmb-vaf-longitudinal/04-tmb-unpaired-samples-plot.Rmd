...
print(ggplot(df_plot, aes(tumor_descriptor, log10_tmb, color = Timepoint)) + 
        ...
        labs(x = "Tumor descriptor",
             y = "log10TMB") +
        theme_Publication() +
        scale_y_continuous(limits = c(log10_ylim_min, (log10_ylim_max + 0.5))) +
        theme(axis.text.x = element_text(angle = 90)))
...
```{r summary-statistics, fig.width = 10, fig.height = 5, fig.fullwidth = TRUE}
...
for (i in seq_along(cancer_groups)) {
  ...
  if(i == 1){
    ...
    p <- print(ggboxplot(df_sub,
          ...
          scale_y_continuous(limits = c(log10_ylim_min, (max(stat.test$y.position)+ 0.5))) +
