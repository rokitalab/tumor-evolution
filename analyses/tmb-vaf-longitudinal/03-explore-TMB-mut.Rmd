...
for (i in seq_along(cancer_groups)) {
  ...
  if (i == 1) {
    ...
    # Name plots
    fname <- paste0(plots_dir, "/", cancer_groups[i], "-Mutations-dumbbell", ".pdf")
    print(fname)
...
```{r create-jitter-ct, fig.width = 18, fig.height = 10, fig.fullwidth = TRUE}
cancer_groups <- unique(as.character(df_plot_filter$cancer_group_sum))
cancer_groups <- sort(cancer_groups, decreasing = FALSE)
print(cancer_groups)
...
for (i in seq_along(cancer_groups)) {
  ...
  if (i == 1) {
    ...
    # Name plots
    fname <- paste0(plots_dir, "/", cancer_groups[i], "-TMB-jitter", ".pdf")
    print(fname)
...
It is not... i have tried with side-by-side barplots and it was the same issue.
Dumbbell plots might be more informative to show changes across timepoints. 
We could consider to use them in later analyses.
...
Maybe we can divide by names(samples), adjust ylim per each and create 3-4 panels for each ylim.

```{r create-barplot-sample-panel, fig.width = 25, fig.height = 20, fig.fullwidth = TRUE}
for (i in seq_along(cancer_groups)) {
  print(i)
  df_ct_sub <- df_plot_filter %>% 
    filter(cancer_group_sum == cancer_groups [i])
