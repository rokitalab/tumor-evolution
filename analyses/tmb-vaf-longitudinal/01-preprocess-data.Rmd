...
maf_autopsy <- patient_list_metadata %>% 
  ...
  mutate(match_id = paste(Kids_First_Biospecimen_ID, tumor_descriptor, sep = "_")) %>%
  unique () 
...
# Count number of bs samples per sample and tumor descriptor
count_bs <- maf_autopsy %>% 
  mutate(match_id = paste(Kids_First_Participant_ID, tumor_descriptor, sep = "_")) %>%
  select(match_id, Kids_First_Biospecimen_ID) %>%
   unique() %>%
  group_by(match_id) %>%
  count() %>%
  dplyr::rename("count" = "count_biospecimen")

# Add count information in the maf df
maf_autopsy_new <- maf_autopsy %>% 
  mutate(match_id = paste(Kids_First_Participant_ID, tumor_descriptor, sep = "_"),
         match_id2 = paste(Kids_First_Biospecimen_ID, tumor_descriptor, sep = "_"))  %>%
  left_join(count_bs, by = "match_id") 


maf_autopsy_new$count_description <- ifelse(maf_autopsy_new$count > 1,"multiple", "single")
  
maf_autopsy_new <- maf_autopsy_new %>%
  write_tsv(file.path(scratch_dir, "maf_autopsy_new2.tsv"))
