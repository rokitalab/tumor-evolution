...
source(paste0(analysis_dir, "/util/function-create-timepoint.R"))
```
...
maf.all <- data.table::fread(maf_file, data.table = FALSE) %>% 
...
# First we will create identifiers to code for multiple timepoints, 
# i.e., "timepoints_all_number" based on "timepoints_all" to account for time points = 1, 2, 3.
maf_autopsy <- patient_list_metadata %>% 
  left_join(maf.all, by = "Kids_First_Biospecimen_ID") %>%
  ...
  select(Kids_First_Participant_ID, Kids_First_Biospecimen_ID, tumor_descriptor, experimental_strategy, timepoints_all, timepoints_all_number, Hugo_Symbol, gene_protein, VAF) %>%
  mutate(match_id = paste(Kids_First_Biospecimen_ID, tumor_descriptor, gene_protein, VAF, sep = "_"),
         match_id_bs = paste(Kids_First_Biospecimen_ID, tumor_descriptor, sep = "_")) %>%
  unique () 

# Next, we will create df when timepoint = 1, 2, 3.
# That we will allow us to will create a column to identify time points to use for the plots functions (timepoints).

# Subset to cases with timepoint 1 
timepoint_df_1 <- maf_autopsy %>%
  mutate(col_rm = ifelse(tumor_descriptor == "Deceased" & timepoints_all_number %in% c("2", "3"), "remove", "keep")) %>%
  filter(col_rm == "keep") %>%
  subset(select = -c(col_rm)) 

# Subset to Deceased and keep timepoint 2 and 3
maf_autopsy.filter <- maf_autopsy %>%
  filter(tumor_descriptor == "Deceased",
         timepoints_all_number %in% c("2", "3"))

df <- unique(maf_autopsy.filter$timepoints_all_number)

for (i in seq_along(df)) {
  print(i)
  maf.sub <- maf_autopsy.filter %>%
    filter(timepoints_all_number == df[i])
  timepoints <- unique(maf.sub$timepoints_all_number)
  print(timepoints)
  
  for (j in seq_along(timepoints)){
    timepoint_df_name <- paste0("timepoint_df_", timepoints[j])
    print (timepoint_df_name)
    p <- create_timepoint(timepoint_df = maf.sub, timepoint = timepoints[j], df_id = df[i])
    print(p)
    assign(timepoint_df_name, p)
  }
}

# Bind df 
maf_autopsy.bind <- rbind(timepoint_df_1, timepoint_df_2, timepoint_df_3) %>%
  mutate(timepoints = case_when(timepoints_all == "dx" & tumor_descriptor %in% c("Diagnosis", "Deceased") ~ "Diagnosis",
         ...
         tumor_descriptor = case_when(tumor_descriptor %in% c("Deceased_1", "Deceased_2", "Deceased_3") ~ "Deceased",
                                      TRUE ~ tumor_descriptor),
         match_id_kids = paste(tumor_descriptor, Kids_First_Participant_ID, sep = "_"),
         match_id_bs = paste(tumor_descriptor, Kids_First_Biospecimen_ID, sep = "_"),
         timepoints_dec = case_when(grepl("Diagnosis", match_id_bs) ~ "1",
...
# We have multiple biospecimen samples per tumor descriptor and per Kids_First_Participant_ID
# First, let's count number of bs samples per sample and tumor descriptor
count_bs <- maf_autopsy.bind %>% 
  mutate(match_id_kids = paste(tumor_descriptor, Kids_First_Participant_ID, sep = "_")) %>%
  select(match_id_kids, Kids_First_Biospecimen_ID) %>%
  unique() %>%
  group_by(match_id_kids) %>%
  count() %>%
  dplyr::rename("count_biospecimen" = "n")

# Add count information in the maf df
maf_autopsy_bs <-maf_autopsy.bind %>% 
  left_join(count_bs, by = "match_id_kids") %>%
  group_by(Kids_First_Participant_ID, tumor_descriptor, Kids_First_Biospecimen_ID) %>% 
  select(Kids_First_Participant_ID, tumor_descriptor, Kids_First_Biospecimen_ID, everything()) %>%
  arrange(Kids_First_Participant_ID) %>%
  write_tsv(file.path(scratch_dir, "maf_autopsy_bs.tsv"))
```
