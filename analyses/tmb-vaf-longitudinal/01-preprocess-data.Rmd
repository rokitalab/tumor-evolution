...
### Data used 
In this notebook, we calculate MAF and create TMB df for coding genes across the longitudinal samples from the PBTA cohort.
...
tmb_missing_bs <- pbta %>% 
  left_join(tmb, by = c("Kids_First_Biospecimen_ID", "experimental_strategy")) %>% 
  filter(is.na(tmb)) %>%
  write_tsv(file.path(results_dir, "tmb_missing_bs.tsv"))

# How many biospecimen samples are missing?
biospecimen_missing <- length(unique(tmb_missing_bs$Kids_First_Biospecimen_ID))
...
tmb_genomic_filter <- paired_list %>% 
  left_join(tmb_genomic, by = "Kids_First_Participant_ID") %>%
  write_tsv(file.path(scratch_dir, "tmb_genomic.tsv"))

tmb_paired_samples_filter <- length(unique(tmb_genomic_filter$Kids_First_Participant_ID))
```
...
There are `r biospecimen_missing` biospecimen samples missing from the TMB file.
