...
paired_time_samples_autopsy <- length(unique(patient_list_metadata$Kids_First_Participant_ID))
```

## Number of patient cases with paired time and autopsy samples

`r paired_time_samples` of patient cases have paired time samples in genomic assays and `r paired_time_samples_autopsy` have autopsy sample.
...
# We will create unique identifiers for each case that will enable to create one plot for each case.
maf_autopsy <- patient_list_metadata %>% 
  ...
  select(Kids_First_Participant_ID, Kids_First_Biospecimen_ID, tumor_descriptor, experimental_strategy, Hugo_Symbol, gene_protein, VAF) %>%
...
# Create tmb file for autopsy samples

```{r tmb_autopsy}
# We will create similat unique identifiers as we did for the maf file.
# We will add timepoint_model column.
tmb_autopsy <- patient_list_metadata %>% 
  left_join(tmb, by = "Kids_First_Biospecimen_ID") %>%
  filter(!is.na(Kids_First_Participant_ID),
         !is.na(tmb)) %>%
   mutate(match_id_bs = paste(tumor_descriptor, Kids_First_Biospecimen_ID, sep = "_"),
         ...
         timepoints_other = case_when(grepl("Deceased", match_id_bs) ~ "4",
                                     TRUE ~ timepoints_other),
         timepoints_model = case_when(descriptors == "Deceased, Diagnosis, Diagnosis, Progressive, Progressive" ~ "Diagnosis_Progressive", 
                                     descriptors == "Deceased, Diagnosis, Recurrence" ~ "dx_rec", descriptors %in% c("Deceased, Progressive, Recurrence", "Deceased, Progressive, Progressive, Recurrence", "Deceased, Progressive, Recurrence, Recurrence") ~ "Progressive_Recurrence",
                                     descriptors == "Deceased, Diagnosis, Progressive, Recurrence" ~ "Diagnosis_Progressive_Recurrence",
                                     descriptors %in% c("Deceased, Diagnosis", "Deceased, Diagnosis, Diagnosis", "Deceased, Diagnosis, Diagnosis, Diagnosis, Diagnosis") ~ "Diagnosis",
                                     descriptors %in% c("Deceased, Progressive", "Deceased, Progressive, Progressive") ~ "Progressive",
                                     descriptors == "Deceased, Recurrence" ~ "Recurrence"),
         log2_mutation_count = log2(tmb_autopsy$mutation_count)) %>%
  write_tsv(file.path(scratch_dir, "tmb_autopsy.tsv"))

# How many patients have tmb data for the paired time samples with autopsy time point present?
paired_time_samples_autopsy_tmb <- length(unique(tmb_autopsy$Kids_First_Participant_ID))
```

## Number of patient cases with paired time and autopsy samples and TMB data

There are `r paired_time_samples_autopsy_tmb` patient cases with TMB information.
