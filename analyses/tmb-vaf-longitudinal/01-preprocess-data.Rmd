...
maf <- readr::read_tsv(maf_file, guess_max = 100000, show_col_types = FALSE) %>%
...
maf %>% 
  write_tsv(file.path(scratch_dir, "maf.tsv"))
...
tmb_vaf_df <- join_all(list(paired_list, tmb_genomic, maf), by = "Kids_First_Participant_ID", type = "left") %>%
  write_tsv(file.path(scratch_dir, "tmb_vaf_genomic.tsv"))
