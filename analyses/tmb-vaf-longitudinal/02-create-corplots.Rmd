...
maf_autopsy <- readr::read_tsv(maf_autopsy_file, guess_max = 100000, show_col_types = FALSE) %>% 
  ...
  mutate(id = paste(Kids_First_Participant_ID, cancer_group, sep = "_"),
         cg_sum = n_distinct(cancer_group) > 1) # to identify samples with different diagnosis across timepoints
...
# Save results by cancer_group
cg_names <- unique(as.character(maf_autopsy$cancer_group))
print(cg_names)

# Loop through cancer_group
for (x in 1:length(cg_names)){
  print(x)
  
  cg_maf_sub <- maf_autopsy %>%
    filter(cancer_group == cg_names[x])
  ...
  print(timepoints_deceased_plots)
   ...
   # for (y in 1:length(maf_sub$cg_sum)){
  #  if (maf_sub$cg_sum[y] == "FALSE"){
   #   print ("Single diagnosis across timepoints")
  #    } else {
   #     print ("Multiple diagnosis across timepoints")
   #     next
   #   }
