...
# Loop through cg_sum
for (names in seq_along(cg_names)){
  print(names)
  
  cg_maf_sub <- maf_autopsy %>%
    filter(cg_sum == cg_names[names])
  ...
  cg_plots_dir <- file.path(plots_dir, cg_names[names])
  if (!dir.exists(cg_plots_dir)) {
    dir.create(cg_plots_dir)
  }
  
  # define cg_sum specific results dir
  cg_results_dir <- file.path(results_dir, cg_names[names])
