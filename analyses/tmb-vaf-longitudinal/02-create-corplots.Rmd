...
# Run corplot
  for (t in seq_along(timepoints_deceased_plots)){
    for (k in seq_along(timepoints_other_plots)){
      ...
      print(fname)
      table_name <- paste0(results_dir, "/", samples[i], "-", timepoints_other_plots[k], "-vs-", timepoints_deceased_plots[t], "-maf_gene_protein.tsv")
      print(table_name)
      p <- create_corplot(maf = maf_sub,
                          ...
                          sid = samples[i])
      write_tsv(maf_join, file = table_name)
