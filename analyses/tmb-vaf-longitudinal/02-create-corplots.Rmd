    ...
    for (t in seq_along(timepoints_deceased_plots)){
      for (k in seq_along(timepoints_other_plots)){
        ...
        p <- create_corplot(maf = maf_sub,
                            ...
                            sid = samples[i])
        print(p)
        # Assign name to plot files and save
...
list_df_bind <- bind_rows(df_list) %>% 
  write_tsv(file.path(results_dir, "all-maf-join-gene-protein.tsv"))
```
