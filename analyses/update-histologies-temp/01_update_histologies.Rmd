...
v12_df <- readr::read_tsv(v12_file, guess_max = 100000, show_col_types = FALSE) %>% 
    select(Kids_First_Participant_ID, cancer_group) #molecular_subtype, integrated_diagnosis, harmonized_diagnosis
...
df <- readr::read_tsv(plot_mapping_file, guess_max = 100000, show_col_types = FALSE) %>% 
  select(broad_histology, plot_group) %>% 
  left_join(hist_df, by = "broad_histology", relationship = "many-to-many") %>% 
  left_join(v12_df, by = "Kids_First_Participant_ID", relationship = "many-to-many") %>% 
