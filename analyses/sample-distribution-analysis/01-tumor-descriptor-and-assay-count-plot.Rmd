...
pbta <- readr::read_tsv(v12_file, guess_max = 100000, show_col_types = FALSE) %>%
  ...
  mutate(tumor_descriptor = case_when(grepl("C4106355-Tumor", sample_id) ~ "Recurrence", # to be updated in v13 as "Recurrence"
         ...
         match_id = paste(tumor_descriptor, sample_id, sep = "_")) %>% # unique identifiers per timepoint and sample related to assay
  write_tsv(file.path(results_dir, "pbta.tsv"))

print(pbta %>% count(tumor_descriptor))

kids_pbta_samples <- print(length(unique(pbta$Kids_First_Participant_ID)))
kids_pbta_samples_timepoint <- print(length(unique(pbta$match_id)))

```

## Number of patient cases

`cohort PBTA kids_pbta_samples` of patient cases in the PBTA cohort and `cohort kids_pbta_samples_timepoint` of patient cases in the PBTA cohort with multiple samples per assay.
...
assays <- pbta %>%
  select(Kids_First_Participant_ID, tumor_descriptor, match_id, experimental_strategy, cancer_group) %>%
  unique() %>%
...
paired_assays_all <- assays %>%
  ...
  mutate(DNA = case_when(grepl("WGS", experimental_strategy) ~ "yes",
                         grepl("WXS", experimental_strategy) ~ "yes", 
...
print(length(unique(genomic_assays_matched_time_points$Kids_First_Participant_ID)))
```
...
We will look only into paired asssays now.
