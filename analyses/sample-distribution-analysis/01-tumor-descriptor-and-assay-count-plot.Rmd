...
v12 <- readr::read_tsv(v12_file, guess_max = 100000, show_col_types = FALSE)
pbta <- readr::read_tsv(v12_file, guess_max = 100000, show_col_types = FALSE) %>%
  ...
  mutate(tumor_descriptor = case_when(grepl("C4106355-Tumor", sample_id) ~ "Recurrence", # to be updated in v13 as "Recurrence"
         ...
         experimental_strategy_sum = case_when(grepl("RNA-Seq", experimental_strategy) & grepl("WGS|WXS", experimental_strategy) ~ "DNA,RNA",
...
```{r assays, echo=TRUE}
assays <- pbta %>%
  select(Kids_First_Participant_ID, tumor_descriptor, match_id, experimental_strategy, cancer_group, experimental_strategy_sum, DNA, match_id_DNA) %>%
  unique() %>%
...
paired_assays_all <- assays %>%
  group_by(Kids_First_Participant_ID, tumor_descriptor, match_id, experimental_strategy, cancer_group, experimental_strategy_sum, DNA, match_id_DNA) %>%
...
kids_pbta_samples_DNA <- print(length(unique(genomic_assays$Kids_First_Participant_ID)))
```
