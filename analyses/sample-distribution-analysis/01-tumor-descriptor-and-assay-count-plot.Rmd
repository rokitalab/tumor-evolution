...
There are `r kids_pbta_samples` of patient cases in the PBTA cohort with timepoint information. 
...
There are `r kids_pbta_samples_DNA` of patient cases in the PBTA cohort with genomic assays.
...
There are `r matched_samples_genomic` of patient cases in the PBTA cohort with matched longitudinal genomic assays.
...
There are `r kids_pbta_samples_RNA` of patient cases in the PBTA cohort with transcriptomic assays.
...
```{r}
transcriptomic_assays_matched_time_points <- transcriptomic_assays %>%
  filter(descriptors %in% c("Diagnosis, Recurrence", "Deceased, Diagnosis", "Diagnosis, Progressive",
                            "Deceased, Recurrence", "Progressive, Recurrence", "Deceased, Progressive", "Deceased, Diagnosis, Progressive")) %>%
  write_tsv(file.path(results_dir, "transcriptomic_assays_matched_time_points.tsv"))

transcriptomic_assays_matched_time_points %>%
  group_by(Kids_First_Participant_ID, descriptors, experimental_strategy) %>%
  tally() %>%
  arrange(desc(n)) %>%
  regulartable() %>%
...
matched_samples_transcriptomic <- length(unique(transcriptomic_assays_matched_time_points$Kids_First_Participant_ID))
```

There are `r matched_samples_transcriptomic` of patient cases in the PBTA cohort with matched longitudinal transcriptomic assays.
...
```
There are `r kids_pbta_samples_paired` of patient cases in the PBTA cohort have paired genomic and transcriptomic assays.

### All time points with paired genomic and transcriptomic assays
We will look at samples with both assays.
...
```{r}
paired_assays_matched_time_points <- paired_assays %>%
  filter(descriptors %in% c("Diagnosis, Recurrence", "Diagnosis, Progressive", "Deceased, Diagnosis", 
                            "Recurrence, Second Malignancy", "Progressive, Recurrence", "Second Malignancy", 
                            "Diagnosis, Second Malignancy" , "Deceased, Progressive, Recurrence", "Diagnosis, Progressive, Recurrence",
                            "Deceased, Recurrence", "Deceased, Diagnosis, Recurrence", "Diagnosis, Unavailable")) %>%
  write_tsv(file.path(results_dir, "paired_assays_matched_time_points.tsv"))

paired_assays_matched_time_points %>%
  group_by(Kids_First_Participant_ID, descriptors, experimental_strategy) %>%
  tally() %>%
  arrange(desc(n)) %>%
  regulartable() %>%
...
matched_samples_paired <- length(unique(paired_assays_matched_time_points$Kids_First_Participant_ID))
```

There are `r matched_samples_paired` of patient cases in the PBTA cohort have with matched longitudinal genomic and transcriptomic assays.
...
These are: Low-grade glioma, Medulloblastoma, Diffuse midline glioma, High-grade glioma, and Ependymoma.
