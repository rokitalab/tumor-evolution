...
samples <- unique(maf_autopsy.filter$timepoints_all_number)

for (i in seq_along(samples)) {
  print(i)
  maf.sub <- maf_autopsy.filter %>%
    filter(timepoints_all_number == samples[i])
  ...
  for (j in seq_along(timepoints)){
    ...
    p <- create_timepoint(timepoint_df = maf.sub, timepoint = timepoints[j], sid = samples[i])
    print(p)
  }
}
...
create_timepoint <- function(timepoint_df, timepoint, timepoints_step1_id) {
 ...
 for (i in seq_along(timepoints_step1)) {
   ...
   for (j in seq_along(timepoints)){
     timepoint_df_name <- paste0(timepoints_step1[i], "timepoint_df_", timepoints[j])
     print (timepoint_df_name)
