...
# Exclude timepoint 1 and run `create_timepoint' function for timepoints 2 and 3
df_exclude_timepoint1 <- maf_autopsy_subset %>%
    filter(!timepoints_all_number == "1")
timepoints <- unique(df_exclude_timepoint1$timepoints_all_number)
print(timepoints)

for (j in seq_along(timepoints)){
  timepoint_df_name <- paste0("timepoint_", timepoints[j])
  print (timepoint_df_name)
  create_timepoint(timepoint_df = df_exclude_timepoint1, timepoint = timepoints[j])
}
